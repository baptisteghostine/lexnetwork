...
import { remarkUnderline } from "@/lib/notes/underline";

// Renders note markdown. mention:// links become in-app links; images
// (attachment URLs) render inline and constrained; `<u>` is the one HTML
// tag honoured (lib/notes/underline.ts) — raw HTML is otherwise inert.
export function Markdown({ children }: { children: string }) {
  return (
    <div className="prose-note text-[13px] leading-relaxed [&_a]:text-blue-400 [&_a:hover]:underline [&_code]:rounded [&_code]:bg-accent [&_code]:px-1 [&_code]:font-mono [&_code]:text-[12px] [&_h1]:text-base [&_h1]:font-semibold [&_h2]:text-sm [&_h2]:font-semibold [&_h3]:text-[13px] [&_h3]:font-semibold [&_img]:my-2 [&_img]:max-h-80 [&_img]:max-w-full [&_img]:rounded-md [&_li]:ml-4 [&_ol]:list-decimal [&_p]:my-1.5 [&_pre]:my-2 [&_pre]:overflow-x-auto [&_pre]:rounded-md [&_pre]:bg-accent [&_pre]:p-2 [&_table]:my-2 [&_td]:border [&_td]:border-border [&_td]:px-2 [&_td]:py-0.5 [&_th]:border [&_th]:border-border [&_th]:px-2 [&_th]:py-0.5 [&_u]:underline [&_u]:underline-offset-2 [&_ul]:list-disc [&_blockquote]:border-l-2 [&_blockquote]:border-border [&_blockquote]:pl-3 [&_blockquote]:text-muted-foreground">
      <ReactMarkdown
